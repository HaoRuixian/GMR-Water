function [res] = bspline_jsnorough(coefs_0,t1_all,sinelv1_all,snr1_all,knots,bspline_order,...
    satno_all,gps,glo,gal,bds,antno_all,meanhgts)
...
for k=1:numel(ants)
    ...
    if gps==1
        ...
        L1k=(2*pi)/L1car;
        modelSNR1 = (coefs_0(end)*sin((4*pi*h1(gps).*sinelv1_all(gps))/L1car)+...
            coefs_0(end-1)*cos((4*pi*h1(gps).*sinelv1_all(gps))/L1car));
    ...
    if glo==1
        ...
        L1car=L1car.';
        L1k=(2*pi)./L1car;
        modelSNR1 = (coefs_0(end-(tmpc-1)*2)*sin((4*pi*h1(glo).*sinelv1_all(glo))./L1car)+...
            coefs_0(end-(tmpc-1)*2-1)*cos((4*pi*h1(glo).*sinelv1_all(glo))./L1car));
    ...
    if gal==1
        ...
        L1k=(2*pi)/L1car;
        modelSNR1 = (coefs_0(end-(tmpc-1)*2)*sin((4*pi*h1(gal).*sinelv1_all(gal))/L1car)+...
            coefs_0(end-(tmpc-1)*2-1)*cos((4*pi*h1(gal).*sinelv1_all(gal))/L1car));
    ...
    if bds==1
        ...
        %L1car=(299792458/(1575.42e06/1.023)); % for Galileo
        L1car=299792458/1575.42e06; % for bds
        L1k=(2*pi)/L1car;
        ...
        res = [res;modelSNR1-snr1_all(bds)];
    end
end

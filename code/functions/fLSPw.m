...
%
%  This is a very efficient MATLAB implementation of the Press-Rybicki approximation of the
%  Lomb-Scargle's periodogram estimator.
%
% INPUT
% x - time vector
% y - amplitude vector
% f - frequency vector which define the frequency bands of interest
...
fmax=f(end); % maximum frequency
fmin=f(1); % minimum frequency
n=length(y);
ave=mean(y);
vr=var(y);
xmin=min(x);
xmax=max(x);
xdif=xmax-xmin;
ofac=max(iofac,1/(xdif*fmin)); % if the lowest frequency is not compatible with the default oversampling factor, redefine it
df = 1/(xdif*ofac);
...
ndim=nfreq; % original code has ndim=2*nfreq
...
for n=1:(length(f)-1)
    ...
    z0(n)=log(length(fr{n}))-log(p0); % approximate threshold for spectral density peaks. Assumes p0<<1

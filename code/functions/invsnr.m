...
for ll = 1:numel(snrdir)
    ...
    for m = 1:mlen
        ...
        if exist([char(snrdir(ll)),'/', station, num2str(tdatenumt),'.mat'],'file')==2
            load([char(snrdir(ll)),'/', station, num2str(tdatenumt),'.mat'])
...
knots = [tdatenum*ones(1,p) ...
    ...
    (tdatenum+tlen)*ones(1,p)]; % knot vector
nsfac = tlen/kspac + p;
